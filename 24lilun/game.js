var game_type1  = "No time limit";
var game_type2  = "Beat the clock";
...
function game_draw(isclock){ // 0) status1, 1) game_type, 2) time, time_left, 3) solved, unsolved, 4) this game history
	if (status1 == 0){
		...
		draw_text (Array(xywh_init1[0] + xywh_init1[2] / 5, xywh_init1[1] + xywh_init1[3] /1.9),game_type1, "#0a0",  "bold " + Math.round(xywh_init1[2]/10) +"px sans-serif");
		return;
	}
	...
	draw_text(Array(rect_quit[0], rect_quit[1] + rect_quit[3]/1.6), "      DONE", "#f00", "bold " + Math.round(rect_quit[2]/6) +"px sans-serif");
...
function onclick1 (e){
	...
	switch (status1){
		case  0: // on the init-game screen
			...
			game_type = click_ID(x,y, rect_arr);
			if (game_type>=0)
				start_game ();
		break;
		case 1:
