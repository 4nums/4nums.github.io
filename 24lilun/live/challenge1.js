...
var TOTAL_TIME;
var over_flag;
var game_type2  = " No divisions";
var game_type1  = "     No  \u00D7 or \u00F7";
...
function gameover(){// quit or finished all
	if (over_flag == 1 && game_STAT == 1){ 
		document.getElementById("gamesub").gamestatus.value =  1;
	}
	else 
		document.getElementById("gamesub").gamestatus.value =  0;
	...
	if (regionID <4) { // click on one of the numbers
		...
		if (numrect_id < 0  || num_focus == regionID ){ return; }  // the region is gone already, or clicking on the already focused num
			...
			if (op_focus < 0) {num_focus = regionID;}  // no op is chosen yet, change num_focus
				...
				if (op_focus == 3 && Math.abs( quad[numrect_id])< ep) { return; }  // divided by 0
					...
					past_steps.push(Array(quad.slice(0), quad_pos.slice(0), num_focus, numfocus_id, numrect_id, op_focus));
		...
		if (past_steps.length>0 || op_focus >=0){ // there is something to undo
			if (op_focus >=0) {op_focus = -1;}  // change the choice of op
				...
				num_prev = last_quads[3]; 
				num_next = last_quads[4]; 
				op_temp = last_quads[5]; 
				future_steps.push(Array(quad.slice(0), quad_pos.slice(0),num_focus,num_prev, num_next,op_temp));
				quad = last_quads[0]; 
		...
		if (future_steps.length >0){
			...
			op_temp = next_quads[5]; 
			num_next = next_quads[4]; 			
			num_prev = next_quads[3]; 			
			past_steps.push(Array(quad.slice(0), quad_pos.slice(0),num_focus, num_prev, num_next, op_temp));
			quad = next_quads[0]; 
			quad_pos = next_quads[1];
			num_focus = next_quads[2];
			op_focus = next_quads[3];
		}
	}
...
function init_game(game_input,  TOTAL_TIME1, game_STAT1, message_str1){
		document.getElementById("gamesub").gamestatus1.value =  0;
		document.getElementById("gamesub").gamestatus2.value =  0;

	game_STAT = game_STAT1;
	message_str = message_str1;
	g_type = 0;
	over_flag = 0;
	quad_all = new Array();
	quad_all.push(game_input);
	quad_all.push('df');
	game_str = 1;  	
...
//	setInterval(function(){tickclock()},10);
	setTimeout(function(){tickclock()},10);
...
function tickclock(){
	...
	if (status1 == 1 || status1 ==3){ // status1 = 3 then it's waiting for the decision on quit or not
		...
		game_tick +=a;
		if (( elapsedTime - 10)>0)
		{	correct = ( elapsedTime - 10); }
		if (game_tick % 100 ==0){
			
			if (game_type ==0){ 
				time_now = TOTAL_TIME - Math.floor(game_tick/100); 
	...
	setTimeout(function(){tickclock()},10 - correct);
...
function game_draw(isclock){ // 0) status1, 1) game_type, 2) time, time_left, 3) solved, unsolved, 4) this game history
	if (game_STAT == 0)
	{
		...
		draw_text(Array(rect_score[0]/2, rect_score[1] + rect_score[3]/1.9), "计算所有玩家的时间!", "#000", "bold " + Math.round(rect_score[2]/5) +"px sans-serif");
		...
		draw_text(Array(rect_clock[0], rect_clock[1] + rect_clock[3]/1.9), "时间: " + time_now, colortext, "bold " + Math.round(rect_clock[2]/5) +"px sans-serif");
		return; 
	}
	if (game_STAT == 2)
	{
		draw_rect(Array(0,0, width, height), "#fff", 1, "#000"); // clean the whole region
		draw_text(Array(rect_score[0]/3, rect_score[1]*2/3 + rect_score[3]*0), "本次游戏的前5名玩家:", "#000", "bold " + Math.round(rect_score[2]/5) +"px sans-serif");
		var ranks = message_str.split(",");
		if (ranks.length ==1)
		{  
			var tmp1 = "无人做出本题!";
			draw_text(Array(rect_score[0]/3, rect_score[1]*2/3 + rect_score[1]*1/3), tmp1, "#00f", "bold " + Math.round(rect_score[2]/5) +"px sans-serif");
		}
		else{  var ii; var tmp1;
			for (ii = 0; ii < ranks.length - 1; ii++)
			{ tmp1 = "["+(ii+1)+"]     "+ranks[ii];
				draw_text(Array(rect_score[0]/3, rect_score[1]*2/3 + rect_score[1]*(1+ii)/4), tmp1 , "#00f", "bold " + Math.round(rect_score[2]/5) +"px sans-serif");
			}
		}
		draw_rect(Array(rect_clock[0],rect_clock[1],rect_clock[2] * 1.3,rect_clock[3]) ,  "#fff", 0, "#00f");
		var colortext = "#000"; if (time_now <=20 && game_type ==0) colortext ="#f00";
	...
	{
		draw_rect(Array(0,0, width, height), "#fff", 1, "#000"); // clean the whole region
		draw_text(Array(rect_score[0]/4, rect_score[1] + rect_score[3]/1.9), "无法连接服务器，请稍后再试！", "#000", "bold " + Math.round(rect_score[2]/5) +"px sans-serif");
		return; 
	}
	if (status1 == 0){
		draw_rect(Array(0,0, width, height), "#fff", 1, "#000"); // clean the whole region
		draw_rect(xywh_init1, "#ccc", 5, "#0f0");
		draw_rect(xywh_init,  "#ccc", 5, "#00f");
		draw_text (Array(xywh_init[0] + xywh_init[2] / 5, xywh_init[1] + xywh_init[3] /1.9),game_type2, "#00a",  "bold " + Math.round(xywh_init[2]/10) +"px sans-serif");
		draw_text (Array(xywh_init1[0] + xywh_init1[2] / 7, xywh_init1[1] + xywh_init1[3] /1.9),game_type1, "#0a0",  "bold " + Math.round(xywh_init1[2]/10) +"px sans-serif");
		return;
	}
	...
	draw_text(Array(rect_score[0], rect_score[1] + rect_score[3]/1.9), "请不要作弊!", "#000", "bold " + Math.round(rect_score[2]/5) +"px sans-serif");
		return;	
	}
	draw_rect(Array(0,0, width, height), "#fff", 1, "#000"); // clean the whole region
	draw_rect(Array(rect_clock[0],rect_clock[1],rect_clock[2] * 1.3,rect_clock[3]) ,  "#fff", 0, "#00f");
	draw_text(Array(rect_clock[0], rect_clock[1] + rect_clock[3]/1.9), "时间: " + time_now, "#000", "bold " + Math.round(rect_clock[2]/5) +"px sans-serif");
	draw_rect(Array(rect_score[0],rect_score[1],rect_score[2] * 1.3,rect_score[3]) ,  "#fff", 0, "#00f");
	draw_text(Array(rect_score[0], rect_score[1] + rect_score[3]/1.9), "请不要作弊!", "#000", "bold " + Math.round(rect_score[2]/5) +"px sans-serif");
...
//	draw_text(Array(rect_skip[0], rect_skip[1] + rect_skip[3]/1.6), "      Skip", "#00f", "bold " + Math.round(rect_skip[2]/6) +"px sans-serif");
		...
		if (quad_pos[ii] == num_focus){
			if (quad.length == 1){
				if (Math.abs(quad[0] - 24) < ep){
						document.getElementById("gamesub").gamestatus1.value =  1;
						document.getElementById("gamesub").gamestatus2.value =  24;
						document.getElementById("gamesub").code_string.value = past_steps[0]+","+past_steps[1]+","+past_steps[2];
	...
	if (status1 == 4 && show_sol){ // show the solutions waiting for the esc
		...
		draw_text(Array(rect_no_sol[0] * 0.5, rect_sol[1]  + rect_sol[3] * 0.05 ), "Press  ESC click anywhere or wait  " + ((pause_total1-pause_tick)/100).toFixed(0) + "   seconds", "#888", "bold " + Math.round(rect_no_sol[3]/5) +"px sans-serif");
		draw_text(Array(rect_no_sol[0] * 1.0, rect_sol[1]  + rect_sol[3] * 0.05 * 2.5 ), "Solutions for   ", "#000", "bold " + Math.round(rect_no_sol[3]/4) +"px sans-serif");
		...
		// quad_all_prev[1]
		draw_text(Array(rect_no_sol[0] * 1.4, rect_no_sol[1] * 1.15), "Never show solutions again", "#888", "bold " + Math.round(rect_no_sol[3]/4) +"px sans-serif");
